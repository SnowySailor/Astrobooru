defmodule Philomena.Astrometry.API do
  ...
  defp submit_url(url, time) when is_binary(url) do
    case expired?(time) do
      false ->
        ...
        case Request.post("url_upload", body, [{"Content-Type", "application/x-www-form-urlencoded"}]) do
          {:ok, data} ->
            Map.get(data, :subid)
          error ->
  ...
  defp wait_for_jobid(subid, time) when is_integer(subid) do
    case expired?(time) do
      ...
      true ->
        {:error, "submission #{subid} was not assigned jobid within #{@retry_timeout/1000} seconds"}
    end
  end
